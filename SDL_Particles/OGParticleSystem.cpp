...
ParticleSystem::ParticleSystem(int numberOfParticles) :ParticleSystem()
{
...
void ParticleSystem::setTexture(std::shared_ptr<LTexture> var)
{
	if (_texture != var)
	{
		//         OG_SAFE_RETAIN(var);
		_texture = nullptr;
		_texture = var;
		//_origin = _texture->getContentSize();
		_dest_rect.size = _texture->getContentSize();
		// updateBlendFunc();
	}
}
...
std::shared_ptr<LTexture> ParticleSystem::getTexture() const
{
	return _texture;
}
...
void ParticleSystem::draw()
{
	...
	for (int i = 0; i < _particleCount; i++)
	{
		...
		Rect dest_render_position = { (p.posx[i] + p.startPosX[i] - p.size[i] / 2) /*+ _render_position.x */,
			(p.posy[i] + p.startPosY[i] - p.size[i] / 2)/*+ _render_position.y*/,
			(p.size[i]), (p.size[i]) };


		SDL_Rect dst = { (int)dest_render_position.origin.x,
			(int)dest_render_position.origin.y,
			(int)dest_render_position.size.width,
			(int)dest_render_position.size.height };


		_texture->render( &dst);

		//	SDL_RenderCopyEx(SDLView::getInstance()->getRenderer(), _texture->getTexture(), nullptr, &r, p.rotation[i], nullptr, SDL_FLIP_NONE);
	}
}
