...
// 
// static LTexture* getDefaultTexture()
// {
...
ParticleExample::ParticleExample(PatticleStyle style)
{
	setStyle(style);
}

bool ParticleExample::setStyle(PatticleStyle style)
{
	if (style == _style)
		return false;
	...
	if (_texture == nullptr)
	{
		std::shared_ptr<LTexture> texture(new LTexture);
		...
		setTexture(texture);
		Size winSize = SDLView::getInstance()->getSize();
		_dest_rect.origin = ((winSize - texture->getContentSize()) / 2);
	}
	...
	case ParticleExample::SNOW:
	{
		...
		setPosVar(Vec2(1.0f *_dest_rect.origin.x, 0.0f));
